...
export async function POST(req: NextRequest) {
  ...
  if (!pending || pending.step !== "TOTP_SETUP_REQUIRED") {
    return NextResponse.json(
      {
        ok: false,
        error: { code: "NO_PENDING_SETUP", message: "Sessione di setup scaduta. Riprova." },
      },
      { status: 401 }
    );
  }
  ...
  if (!adminUser || !adminUser.active || !adminUser.totpSecret) {
    return NextResponse.json(
      { ok: false, error: { code: "INVALID_STATE", message: "Stato non valido." } },
      { status: 400 }
    );
  }
