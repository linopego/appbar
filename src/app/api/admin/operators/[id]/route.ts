...
export async function PATCH(
  ...
  if (id === session.operatorId) {
    return NextResponse.json({ ok: false, error: "Non puoi modificare te stesso" }, { status: 403 });
  }
  ...
  if (body.email !== undefined) {
    const emailVal = body.email === null ? null : typeof body.email === "string" ? body.email.trim() || null : undefined;
